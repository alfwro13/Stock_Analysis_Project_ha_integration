...
Two **Number** entities per holding — **`<ticker>` Low Limit** and **`<ticker>` High Limit**, both disabled by default — set the corresponding limit in the instrument's native currency. Their value is read from and written to the backend (`holding_price_limits` table), not stored locally in Home Assistant, so it stays fully in sync with the same target set from the main app's own Stock Detail page (Position Targets) in both directions — a change on either end shows up on the other within one poll cycle (or immediately, on the end that made the change). **0 clears the limit** — since a real price target can never sensibly be 0, setting the number down to 0 (its minimum) clears that limit on the backend, mirroring the Stock Detail page's own "blank input clears the target" convention; a cleared/never-set limit likewise displays as 0 rather than "unknown". Watchlist-only tickers have no Trading-account holding and so never appear here — they can only be targeted from the Stock Detail page itself.

**Auto-enable/disable:** you don't need to manually enable these entities to use them from the web app — as soon as a target is set for a holding (from either end), its Low/High Limit entity is automatically enabled on the next poll, and if that target is later cleared, the entity is automatically disabled again (at most once per UTC calendar day, to avoid rapid set/clear cycles thrashing the entity registry). This is based purely on whether a target is set on the backend, regardless of the entity's current enabled/disabled state or how it got there — including an entity you'd previously disabled manually from the HA UI, which will be re-enabled if the backend still has (or gets) a target for it. The one exception: an entity you manually enable from the HA UI that has never had a target set is left alone rather than being disabled back out on the next poll, so you can enable it and type in its first target from HA without a race against the next sync.
