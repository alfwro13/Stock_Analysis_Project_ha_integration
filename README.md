...
**Auto-enable/disable:** you don't need to manually enable these entities to use them from the web app — as soon as a target is set for a holding (from either end), its Low/High Limit entity is automatically enabled on the next poll, and if that target is later cleared, the entity is automatically disabled again (at most once per UTC calendar day, to avoid rapid set/clear cycles thrashing the entity registry). This only ever touches entities the integration itself enabled or disabled: a target you manually enable from the HA UI before it's ever had a value is left alone (so you can enable it and set its first target from HA without it being disabled out from under you), and an entity you manually disable from the HA UI stays disabled even if the backend still has a target set for it.
