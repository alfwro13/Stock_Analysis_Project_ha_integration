...
async def test_user_step_defaults_show_toggles_to_true(hass: HomeAssistant, mock_api_for_flow) -> None:
    ...
    assert result["data"].get("show_markets", True) is True
...
async def test_reconfigure_can_disable_show_markets(
    hass: HomeAssistant, mock_config_entry, mock_api_for_flow
) -> None:
    """Reconfigure flow can flip show_markets to False."""
    mock_config_entry.add_to_hass(hass)

    updated_input = {**SAMPLE_CONFIG, "show_markets": False}
    ...
    assert result["type"] == FlowResultType.ABORT
    assert result["reason"] == "reconfigure_successful"
    assert mock_config_entry.data["show_markets"] is False
