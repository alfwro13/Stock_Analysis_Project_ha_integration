...
Phase 6's Markets sensors are keyed by the backend's `market_ticker_registry` row id (`registry_ticker`, the registry's own stable primary ticker), never by the resolved `ticker` field `GET /api/markets` returns per tile. Five dual-instrument indexes (S&P 500, Nasdaq 100, Dow, Russell 2000, Nikkei 225) auto-swap that resolved ticker between a spot symbol and a paired futures symbol (e.g. `^GSPC` ↔ `ES=F`) depending on session (`markets_engine.resolve_tile()`) — keying a sensor's `unique_id` off the resolved value would silently recreate the entity (new unique_id, new entity_id, lost history) every time the session flipped, violating unique_id stability. `assemble_markets_payload()`'s tile payload carries both fields specifically so a consumer needing stable identity has one available — check for an equivalent "resolved value that can change independently of the underlying item's identity" trap before keying any future per-item entity set off a backend API field; prefer whatever field is documented as the row's own primary key over one described as "currently displayed" or "resolved."
